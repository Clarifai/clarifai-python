...
CREATE_COMPUTE_CLUSTER_ID = "test-compute-cluster"
CREATE_NODEPOOL_ID = "test-nodepool"
CREATE_DEPLOYMENT_ID = "test-deployment"
...
class TestComputeOrchestration:
  ...
  def test_get_deployment(self, create_nodepool):
    ...
    assert deployment.id == CREATE_DEPLOYMENT_ID and deployment.nodepool_id == CREATE_NODEPOOL_ID and deployment.compute_cluster_id == CREATE_COMPUTE_CLUSTER_ID and deployment.user_id == CREATE_COMPUTE_CLUSTER_USER_ID
  ...
  def test_delete_deployment(self, create_nodepool, caplog):
    with caplog.at_level(logging.INFO):
      all_deployments = list(create_nodepool.list_deployments())
      create_nodepool.delete_deployments(deployment_ids=all_deployments[0]['id'])
