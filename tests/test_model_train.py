...
class Testmodeltrain:
    ...
    def test_model_params(self):
        ...
        assert (
            expected_data['train_params'].keys() == model_params['train_params'].keys()
        )  # test yaml model params
        assert len(model_params['concepts']) == 0  # test model params concepts
        assert (
            'custom_config' in expected_data['train_params'].keys()
        )  # test custom config in yaml
        assert (
            expected_data['train_params']['template'] == 'MMClassification_AdvancedConfig'
        )  # test template param in yaml
        assert (
            'image_size' in expected_data['train_params'].keys()
        )  # test image size param in yaml
