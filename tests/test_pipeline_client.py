...
class TestPipelineClient:
    ...
    def test_run_failure_to_start(self, mock_init):
        ...
        mock_response = Mock()
        mock_response.status.code = status_code_pb2.StatusCode.MIXED_STATUS  # Use a valid status code
        ...
        with pytest.raises(UserError, match="Failed to start pipeline run: Failed to start pipeline"):
            pipeline.run()
