...
def test_requirements_loosened_constraints():
    ...
    for package, version in loosened_packages.items():
        ...
        assert expected_constraint in content, f"Expected {expected_constraint} in requirements.txt"
        
        # Should NOT have == constraint
        forbidden_constraint = f"{package}=={version}"
        assert forbidden_constraint not in content, f"Should not have exact constraint {forbidden_constraint}"
