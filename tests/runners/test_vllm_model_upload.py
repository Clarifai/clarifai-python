...
@pytest.mark.maintainer_approval
...
@pytest.mark.maintainer_approval
def test_model_uploader_flow(dummy_models_path, caplog):
    """
    ...
    assert builder.config["model"]["id"] == "facebook-opt-125m"
    ...
    # Upload a new version with logging
    logger.info("Starting model version upload...")
    try:
        builder.upload_model_version()
        logger.info("Model version upload initiated successfully")

        # After starting the upload/build, we expect model_version_id to be set
