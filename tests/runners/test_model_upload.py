...
def test_cli_platform_override(tmp_path):
    """
    Test that platform specified via CLI parameter overrides config.yaml platform.
    """
    tests_dir = Path(__file__).parent.resolve()
    original_dummy_path = tests_dir / "dummy_runner_models"

    if not original_dummy_path.exists():
        pytest.skip("dummy_runner_models not found, skipping CLI platform override test")

    # Copy the folder to tmp_path
    target_folder = tmp_path / "dummy_runner_models"
    shutil.copytree(original_dummy_path, target_folder)

    # Update config.yaml with a platform
    config_yaml_path = target_folder / "config.yaml"
    with config_yaml_path.open("r") as f:
        config = yaml.safe_load(f)
    ...
    NOW = uuid.uuid4().hex[:8]
    config["model"]["user_id"] = CLARIFAI_USER_ID
    config["model"]["app_id"] = f"test-platform-override-{NOW}"
    
    # Set platform in config
    if "build_info" not in config:
        config["build_info"] = {}
    config["build_info"]["platform"] = "linux/amd64"

    with config_yaml_path.open("w") as f:
        yaml.dump(config, f, sort_keys=False)

    # Test 1: Platform from config.yaml
    builder_config = ModelBuilder(str(target_folder), validate_api_ids=False)
    model_version_proto_config = builder_config.get_model_version_proto()
    if hasattr(model_version_proto_config.build_info, 'platform'):
    ...
    # Test 2: Platform from CLI override
    cli_platform = "linux/arm64"
    builder_cli = ModelBuilder(str(target_folder), validate_api_ids=False, platform=cli_platform)
    model_version_proto_cli = builder_cli.get_model_version_proto()
    if hasattr(model_version_proto_cli.build_info, 'platform'):
    ...
    # Test 3: CLI platform with multiple platforms
    cli_platform_multi = "linux/amd64,linux/arm64"
    builder_cli_multi = ModelBuilder(
        str(target_folder), validate_api_ids=False, platform=cli_platform_multi
    )
    model_version_proto_multi = builder_cli_multi.get_model_version_proto()
    if hasattr(model_version_proto_multi.build_info, 'platform'):
