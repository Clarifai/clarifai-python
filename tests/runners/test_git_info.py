...
""")
    ...
    def test_get_git_info_git_folder(self):
        ...
        # Initialize git repo
        subprocess.run(['git', 'init'], cwd=self.model_dir, capture_output=True)
        subprocess.run(['git', 'config', 'user.email', 'test@test.com'], 
                      cwd=self.model_dir, capture_output=True)
        subprocess.run(['git', 'config', 'user.name', 'Test User'], 
                      cwd=self.model_dir, capture_output=True)
        subprocess.run(['git', 'add', '.'], cwd=self.model_dir, capture_output=True)
