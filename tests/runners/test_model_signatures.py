...
class TestModelCalls(unittest.TestCase):
    ...
    def test_str_ListImage__str_ListImage(self):
        ...
        self.assertTrue(
            np.all(result_images[0].to_numpy() == np.asarray(ImageOps.invert(testimg1)))
        )
        self.assertTrue(
            np.all(result_images[1].to_numpy() == np.asarray(ImageOps.invert(testimg2)))
        )
    ...
    def test_generate_wrong_return_type(self):
        ...
        client = _get_servicer_client(MyModel())
        with (
            self.assertRaisesRegex(Exception, 'Incompatible type'),
    ...
    def test_stream_named_inputs_str_str__str(self):
        ...
        client = _get_servicer_client(MyModel())
        result = list(
            client.f(stream=iter([NamedFields(x='a', y='b'), NamedFields(x='x', y='y')]))
        )
    ...
    def test_docstrings(self):
        class MyModel(ModelClass):
            ...
            @ModelClass.method
            def stream(
                self, stream: Iterator[NamedFields(x=str, y=str)], n: int
            ) -> Iterator[NamedFields(xout=str, yout=str)]:
    ...
    def test_ListListNamedFieldsImagestrint_type(self):
        class MyModel(ModelClass):
            @ModelClass.method
            def f(
                self, x: List[List[NamedFields(x=str, y=int)]]
            ) -> List[List[NamedFields(x=str, y=int)]]:
        ...
        result = client.g(
            [
                [NamedFields(fld1='a', fld2=1), NamedFields(fld1='b', fld2=2)],
                [NamedFields(fld1='x', fld2=3), NamedFields(fld1='y', fld2=4)],
            ]
        )
