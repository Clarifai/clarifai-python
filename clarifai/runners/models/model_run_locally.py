...
class ModelRunLocally:
                ...
                service_pb2.PostModelOutputsRequest(
                    ...
                    inputs=[
                        resources_pb2.Input(
                            data=resources_pb2.Data(
                                text=resources_pb2.Text(
                                    raw="Generate an image of a dog playing in the park"
                                ),
                            )
                        )
                    ],
    ...
    def _run_model_on_single_request(self, model, request, stream_request):
        ...
        try:
            ...
            traceback.print_exc()
            predict_response = service_pb2.MultiOutputResponse(
        ...
        try:
            ...
            traceback.print_exc()
            stream_response = service_pb2.MultiOutputResponse(
                status=status_pb2.Status(
    ...
    def _run_model_inference(self, model):
        ...
        for req_index in range(len(requests)):
            self._run_model_on_single_request(
                model, requests[req_index], stream_requests[req_index]
            )
        ...
        try:
            ...
            if process and process.poll() is None:
                ...
                try:
                    ...
                    logger.info(
                        "Process did not terminate gracefully. Killing process."
                    )
    ...
    def run_model_server(self, port=8080):
        ...
        try:
            ...
            logger.info(
                "Model server started successfully and running at localhost:{port}"
            )
        except subprocess.CalledProcessError as e:
    ...
    def _docker_hash(self):
        ...
        return hashlib.md5(
            f"{requirements_hash}{dockerfile_hash}".encode("utf-8")
        ).hexdigest()
    ...
    def build_docker_image(
        ...
        try:
            logger.info(
                f"Building docker image from Dockerfile in {self.model_path}..."
            )
    ...
    def run_docker_container(
        ...
        try:
            ...
            cmd.extend(
                ["--model_path", "/home/nonroot/main", "--grpc", "--port", str(port)]
            )
            # Run the container
    ...
    def stop_docker_container(self, container_name="clarifai-model-container"):
        ...
        try:
            ...
            if result.stdout.strip() == container_name:
                logger.info(
                    f"Docker container '{container_name}' is running. Stopping it..."
                )
                subprocess.run(["docker", "stop", container_name], check=True)
                logger.info(
                    f"Docker container '{container_name}' stopped successfully!"
