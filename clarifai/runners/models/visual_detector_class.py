...
from clarifai.runners.utils.data_types import Concept, Frame, Image, Region, Video
...
class VisualDetectorClass(ModelClass):
    ...
    def predict(self, image: Image) -> List[Region]:
        ...
        """
        raise NotImplementedError("Image processing to be implemented in child class")

    @ModelClass.method
    def generate(self, video: Video) -> Iterator[Frame]:
        """Process video frames sequentially and detect objects in each frame.

        This method processes a video input frame by frame, performing object detection
        on each frame and yielding the detection results as they become available.

        Args:
            video: Input video to process, provided as a Video object

        Yields:
            A Frame object containing the processed image data, timestamp and
            detected regions for each processed frame
        """
        raise NotImplementedError("Video processing to be implemented in child class")
