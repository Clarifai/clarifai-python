...
class Inputs(Lister, BaseClient):
  ...
  def _get_proto(
    ...
    if labels:
      if not label_ids:
        concepts = [resources_pb2.Concept(id=_label, name=_label, value=1.0) for _label in labels]
      else:
        ...
        if 'geopoints' in columns:
          if len(input['geopoints']) > 0:
            ...
            geo_info = (
              geo_points
              if len(geo_points) == 2
              else UserError("geopoints column in CSV file should have longitude,latitude")
            )
          else:
            geo_info = None
        else:
  ...
  def get_text_inputs_from_folder(
    ...
    for filename in os.listdir(folder_path):
      ...
      input_protos.append(
        Inputs._get_proto(input_id=input_id, dataset_id=dataset_id, text_pb=text_pb, labels=labels)
      )
    return input_protos
