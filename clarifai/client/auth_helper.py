  ...
  def __init__(
    ...
    if pat == "":
      if os.environ.get("CLARIFAI_PAT", "") == "":
        raise Exception("Need 'pat' in the query params or use one CLARIFAI_PAT as env var")
      else:
...
Please check the following required query params are in the url:
 ...
 - 'pat': to authenticate the calling user with a personal access token.
...
Please check the following required vars are in your env:
 ...
 - 'CLARIFAI_PAT': to authenticate the calling user with personal access token.
...
"""
  ...
  def get_user_app_id_proto(
    ...
    """
        Get the gRPC metadata that contains either the PAT to use.
  ...
  def metadata(self):
    """
        Get the gRPC metadata that contains the PAT to use.
    ...
    if self._pat != "":
      ...
      raise Exception("'pat' needed to be provided in the query params or env vars.")
