...
class TextClassificationDataset(ClarifaiDataset):
  ...
  def _extract_protos(self, batch_input_ids: List[int]) -> Tuple[List[resources_pb2.Input], List[resources_pb2.Annotation]]:
    ...
    def process_datagen_item(id):
      ...
      labels = datagen_item.labels if isinstance(datagen_item.labels, list) else [datagen_item.labels]  # clarifai concept
      ...
      input_protos.append(self.create_input_protos(text, labels, input_id, self.dataset_id, metadata))
