...
class DefaultTestInferenceModel(unittest.TestCase):
  ...
  def triton_get_predictions(self, input_data):
    ...
    """
    return inspect.unwrap(self.triton_model.inference_obj.get_predictions)(self.triton_model.inference_obj, input_data)
  ...
  def intitialize(self, 
    ...
    # load labels
    self._required_label_model_types = ["visual-detector", "visual-classifier", "text-classifier", "visual-segmenter"]
    self.labels = []
  ...
  def test_triton_config(self):
    ...
    self.assertEqual(self.triton_model_input_name, 
                     self.default_triton_model_config.input[0].name,
    ...
    for output_name in self.triton_model_config.output:
      self.assertIn(output_name.name, 
  ...
  def test_having_labels(self):
    if self.model_type in self._required_label_model_types:
      self.assertTrue(len(self.labels), 
  ...
  def test_inference_with_predefined_inputs(self):
    ...
    for inp, output in zip(inputs, outputs):
      ...
      self.assertEqual(len(self.triton_model_config.output[0].dims),
      ...
      if self.model_type == "visual-detector":
        ...
        self.assertTrue(_is_valid_logit(output.predicted_scores), 
                        "`predicted_scores` must be in range [0, 1]")
        self.assertTrue(_is_non_negative(output.predicted_bboxes),
                        "`predicted_bboxes` must be >= 0")
        self.assertTrue(np.all(0 <= output.predicted_labels) 
                        and np.all(output.predicted_labels < len(self.labels)),
                        f"`predicted_labels` must be in [0, {len(self.labels) - 1}]")
        self.assertTrue(_is_integer(output.predicted_labels),
                        "`predicted_labels` must be integer")
        ...
        self.assertTrue(_is_valid_logit(output.predicted_scores),
                        "`predicted_scores` must be in range [0, 1]")
        if self.labels:
          self.assertEqual( len(output.predicted_scores), len(self.labels),
        ...
        self.assertTrue(_is_non_negative(output.image),
                        "`image` elements must be >= 0")
        ...
        self.assertTrue(_is_integer(output.predicted_mask),
                        "`predicted_mask` must be integer")
        if self.labels:
          self.assertTrue(np.all(0 <= output.predicted_mask)
                        and np.all(output.predicted_mask < len(self.labels)),
